...
import { TravelTypeResidenceDataSource } from '../../services/travel-type-residence-datasource.service';
...
export class ListComponent implements OnInit {
  ...
  columnsToDisplay = ['more','name','icon','actions'];
  dataSource: TravelTypeResidenceDataSource;
  ...
  constructor(
    private managerService: TravelTypeResidenceService
  ...
  dataSourceInit(): void {
    this.dataSource = new TravelTypeResidenceDataSource(this.managerService);
    this.dataSource.loadCategories(1, this.pageSize, this.filters);
  }
  ...
  delete(id: number): void {
    ...
    const dialogRef = this.dialog.open(DeleteEntityDialogComponent, {
      data: { id, title, itemName, service, url: '/TravelResidence/Delete' },
      width: '440px'
    });
  ...
  sortData(sort: Sort) {
    ...
    this.dataSource.travelTypeResidence$.subscribe(x => {
      this.seedData = x
    })
    ...
    this.dataSource.travelTypeResidenceSubject.next(data);
  }
}
