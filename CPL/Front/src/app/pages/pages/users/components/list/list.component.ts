...
import { UserDataSource } from '../../services/users-datasource.service';
...
export class ListComponent implements OnInit {
  ...
  columnsToDisplay = ['avatar','displayName' ,'userName', 'isActive', 'actions'];
  dataSource: UserDataSource;
  filters: TableFilterRules[];
  @ViewChild(MatSort, { static: true }) sort: MatSort;
  pageSize: number;
  pageSizeOptions: number[];
  url:string="/admin/user/GetManagerImage/";
  sortedData: GetAllManagerModel[] = [];
  seedData: GetAllManagerModel[] = [];

  constructor(
    private managerService: UsersService
  ...
  dataSourceInit(): void {
    this.dataSource = new UserDataSource(this.managerService);
  ...
  sortData(sort: Sort) {
    ...
    this.dataSource.users$.subscribe(x => {
      this.seedData = x
    })
    ...
    this.dataSource.usersSubject.next(data);
  }
}
