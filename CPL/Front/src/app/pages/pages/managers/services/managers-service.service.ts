import { AddManagerModel, EditManagerModel, GetManagerModel, GetAllManagerModel } from '../models';
...
export class ManagersService extends GenericServic<any, any, any, any>{
    ...
    UpdateAdminRoles(userId: number, rolesId: number[]): Observable<any> {
        const Url = `${this.appConfig.apiEndpoint}/Manager/SetPermissions`;
        return this.httpClient.put(Url, { userId, rolesId });
    }

    VerifyPhonenumber(phoneNumber, id): Observable<any> {
        const Url = `${this.appConfig.apiEndpoint}/Manager/PersonalInfo/PhoneVerification/${id}`;
        return this.httpClient.post(Url, { phoneNumber });
    }

    ChangeAccountInfo(email, confirmEmail, isActive, id): Observable<any> {
        const Url = `${this.appConfig.apiEndpoint}/Manager/UpdateAccountInfo/${id}`;
